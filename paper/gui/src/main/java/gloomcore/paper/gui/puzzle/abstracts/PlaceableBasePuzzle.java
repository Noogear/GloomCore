 ...
 */
public abstract class PlaceableBasePuzzle<C extends Context> extends AbstractPuzzle<C> implements PlaceablePuzzle<C> {
     ...
     */
    @Override
    public void onClick(org.bukkit.event.inventory.InventoryClickEvent event, C owner) {
        event.setCancelled(false);
        if (onContentsChanged != null) {
            // 交给具体视图的实体线程去调度（避免主线程立刻读取未稳定的inventory状态）。
            gloomcore.paper.scheduler.PaperScheduler.INSTANCE
                    .entity(event.getWhoClicked())
                    .runDelayed(() -> onContentsChanged.accept(owner), 1L);
        }
    }

    /**
     * 默认不需要周期更新，由具体子类决定是否覆写。
     */
    @Override
    public void update(C context) {
        // no-op for placeable base by default
    }
    ...
    public void cleanupOnClose(C context, Inventory inventory) {
        ...
        returnItemsToPlayer(context, toReturn);
    }
