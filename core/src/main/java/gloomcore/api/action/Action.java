...
public interface Action<C> {
    ...
    /**
     * 启动此操作链并执行，使用指定的 Logger 进行安全的异常处理。
     * (推荐在插件中使用)
     *
     ...
     */
    default CompletableFuture<C> launch(C initialContext, Logger logger) {
        return this.execute(initialContext)
                .exceptionally(ex -> {
                    logger.log(Level.SEVERE, "Action chain failed with an unhandled exception:", ex);
                    return null;
                });
    }
     ...
     * @return 包含最终结果的 CompletableFuture。如果发生未恢复的异常，结果将为 null。
     */
    default CompletableFuture<C> launch(C initialContext) {
        return this.execute(initialContext)
                .exceptionally(ex -> {
